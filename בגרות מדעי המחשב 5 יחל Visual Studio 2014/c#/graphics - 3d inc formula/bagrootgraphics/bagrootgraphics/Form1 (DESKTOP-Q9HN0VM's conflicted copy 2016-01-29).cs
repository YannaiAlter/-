...
namespace WindowsFormsApplication1
{
    public partial class Form1 : Form
    {
        ...
        //ט. יציאה: הפונקצייה מפעילה לולאה שמחשבת את העומק ונקודות האיקס וואי החדשות לפי נוסחת המרה מדו-מימד לתלת-מימד והפונקצייה המתקבלת
            ...
            if (PaintWithOutLines.Checked == false && PaintWithLines.Checked == false)
            {
                ...
                for (int i = oldXYZ.GetLength(0) - 2; i > 0; i--)
                {
                    for (int j = oldXYZ.GetLength(1) - 2; j > 0; j--)
                    {
                        ...
                        g.DrawLine(Pens.Black, (float)(w + newX[i, j]), (float)(h - newY[i, j]), (float)(w + newX[i, j + 1]), (float)(h - newY[i, j + 1]));
                        g.DrawLine(Pens.Black, (float)(w + newX[i, j]), (float)(h - newY[i, j]), (float)(w + newX[i + 1, j]), (float)(h - newY[i + 1, j]));
                        progressBar1.Value++;
        ...
        //הפונקצייה מציירת את הגרף לפי הנקודות במערך שהוגדר בפונקצייה הקודמת
        public void Fill3D()
        {
            ...
            for (int i = oldXYZ.GetLength(0) - 2; i > 0; i--)
            {
                for (int j = oldXYZ.GetLength(1) - 2; j > 0; j--)
                {
                         ...
                         try
                         {
                             Point p1 = new Point((int)(w + newX[i, j]), (int)(h - newY[i, j]));
                             Point p2 = new Point((int)(w + newX[i, j + 1]), (int)(h - newY[i, j + 1]));
                             Point p3 = new Point((int)(w + newX[i + 1, j + 1]), (int)(h - newY[i + 1, j + 1]));
                             Point p4 = new Point((int)(w + newX[i + 1, j]), (int)(h - newY[i + 1, j]));
        ...
        //הפונקצייה מגדירה מאפיינים התחלתיים
        ...
        private void panel1_MouseUp(object sender, MouseEventArgs e)
        {
            for (int i = 0; i < oldXYZ.GetLength(0); i++)
            {
                for (int j = 0; j < oldXYZ.GetLength(1); j++)
                {
                    newX[i, j] += newX[i, j] - e.X;
                    newY[i, j] += newY[i, j] - e.Y;
                }
            }
